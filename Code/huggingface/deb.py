...
Implementation using Hugging Face Inference API for remote execution
"""
...
# =============================
# 3. Hugging Face Agent implementation
# =============================

class HuggingFaceAgent:
    """Agent powered by Hugging Face Inference API for code reasoning with ToM capabilities"""
    
    def __init__(self, model_name: str, agent_id: int, api_token: str, max_tokens: int = 512):
        self.model_name = model_name
        self.agent_name = f"Agent-{agent_id}({model_name.split('/')[-1]})"
    ...
    def _call_hf_api(self, prompt: str, temperature: float = 0.7) -> str:
...
Respond ONLY with valid JSON."""
    ...
    def initial_response(self, example: CodeExample) -> AgentResponse:
        ...
        raw = self._call_hf_api(prompt, temperature=0.7)
    ...
    def construct_tom(self, target_response: AgentResponse) -> TheoryOfMind:
        ...
        raw = self._call_hf_api(prompt, temperature=0.5)
    ...
    def debate_response(
        ...
        raw = self._call_hf_api(prompt, temperature=0.6)
        prediction, confidence, reasoning = self._parse_response(raw)
...
class MMADFramework:
    ...
    def __init__(
        ...
        api_token: str,
        decider_model: str = "Qwen/Qwen2.5-1.5B-Instruct",
        max_rounds: int = 3
    ):
        """
        ...
        """
        self.agents = [
            HuggingFaceAgent(config["model"], idx + 1, api_token, max_tokens=512)
            for idx, config in enumerate(agent_configs)
        ]
        self.decider = DeciderAgent(decider_model, api_token)
...
def main():
    ...
    if not HF_TOKEN:
        raise ValueError(
            "Please set HF_TOKEN environment variable with your Hugging Face API token.\n"
            "Get one at: https://huggingface.co/settings/tokens\n"
            "Then run: export HF_TOKEN='your_token_here'"
        )
    
    # Use small models available on Hugging Face Inference API
    # These are free-tier compatible models
    AGENT_CONFIGS = [
        {"name": "Qwen", "model": "Qwen/Qwen2.5-1.5B-Instruct"},
        {"name": "Phi", "model": "microsoft/Phi-3-mini-4k-instruct"},
        {"name": "Gemma", "model": "google/gemma-2-2b-it"}
    ]
    
    print("=" * 80)
    print("MMAD Framework for Code Reasoning (Hugging Face Remote)")
    print("=" * 80)
    ...
    mmad = MMADFramework(
        ...
        api_token=HF_TOKEN,
        decider_model="Qwen/Qwen2.5-1.5B-Instruct",
        max_rounds=3
    )
